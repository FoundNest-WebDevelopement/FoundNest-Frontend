...
export default function LostReportMangementModal(
    ...
    const handleCancelReport = async (reason) => {
        ...
        try {
            ...
            const response = await fetchWithAuth(
                `${API_URL}/api/lost-reports/${lostReportId}/cancel`,
                {
                    method: "PUT",
                    body: JSON.stringify({
                        reason,
            ...
            if (!response.ok) {
                throw new Error(data.error || data.message || "Failed to cancel report");
            }
            ...
            if (Array.isArray(reportsData)) {
                ...
                if (updatedReport) setSelectedItem(updatedReport);
            }

            setOpenCancelDialog(false);
            toast.success(`Successfully marked ${formatReportId(lostReportId)} as Cancelled.`);
        } catch (err) {
            console.error(err);
    ...
    return (
        <>
                ...
                <div className="absolute top-0 right-0 h-full w-3/10 bg-white flex flex-col">
                    <div className="w-full h-15 bg-primary items-center flex pl-2 gap-4 shrink-0 sticky top-0 z-50">
                        ...
                        <div className="ml-auto pr-6">
                            <button onClick={checkChanges} className="cursor-pointer"><i className="fa-solid fa-x text-xs xl:text-sm text-white"></i></button>
                        </div>
                    </div>
                    {editTab ?
                        (
            ...
            {openCancelDialog && (
                <CancelReportDialog
                    reportLabel={formatReportId(lostReportId)}
